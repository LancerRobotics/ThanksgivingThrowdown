...
public class TeamScoringElementPipeline implements VisionProcessor { // aka pipeline
    ...
    private final static class FrameOutputContext {
        ...
        private @Nullable TeamScoringElementLocation getLocation() {
            ...
            final @NotNull Optional<CalculatedDetectionZone> maxConfidenceZone = zones.stream().max(Comparator.comparingDouble((z) -> z.getRelativeLitRatio(this)));
            if (maxConfidenceZone.isPresent()) {
                final @NotNull CalculatedDetectionZone detectionZone = maxConfidenceZone.get();
